...
void GameEngine::issueOrdersPhase() {
	...
	*/
	...
	for (Player* p : players) {
		...
		for (auto it : p->toAttack()) {
			cout << it->getIndex() << " : " + it->getName() << endl;
		}

		cout << "\nNow printing out " + p->getName() + "'s list of territories to defend" << endl;

		for (auto it : p->toDefend()) {
			cout << it->getIndex() << " : " + it->getName() << endl;
		}
		...
		while (answer == "yes") {
			...
			cout << endl;
			if (ans == 1) {
				cout << "\nPrinting out " + p->getName() + "'s list of territories to defend" << endl;
				...
				for (auto it : p->toAttack()) { cout << it->getIndex() << " : " + it->getName() << endl; }
				cout << endl;
				cout << "Input the territory index you want to advance FROM: ";
				int sourceIndex;
				cin >> sourceIndex;
		...
		if (index >= 0 || index < p->getHand()->getSize()) {
			cout << "You have chosen card " + p->getHand()->getCardInHand(index)->getCardTypeName();
